...
import { llmDiscoverModelsValueSchema, llmModelsValueSchema, llmProvidersValueSchema, llmStartOAuthLoginValueSchema } from '../api/llm.schema.ts'
import {
...
export interface IApiClient {
  ...
  llm: {
    ...
    startOAuthLogin(payload: RequestPayload<'llm.startOAuthLogin'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'llm.startOAuthLogin'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'llm.startOAuthLogin': llmStartOAuthLoginValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly llm: IApiClient['llm'] = {
    ...
    startOAuthLogin: (payload, signal) => this.callUnary('llm.startOAuthLogin', payload, signal),
  }
