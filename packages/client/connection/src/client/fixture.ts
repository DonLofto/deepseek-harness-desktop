...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    llm: {
      ...
      startOAuthLogin: request => ok(request, { ok: true, provider: request.payload.provider }),
    },
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'llm.startOAuthLogin': return this.api.llm.startOAuthLogin(request, signal)
    }
  }
